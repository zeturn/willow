...
<div class="container mx-auto mt-5 max-w-7xl">
    <!-- Topic Detail Card -->
    <div class="mb-6 bg-white rounded-lg p-6 z-0">
        <nav class="flex justify-between z-1">
            <ol class="inline-flex items-center mb-3 space-x-3 text-sm text-neutral-500 [&_.active-breadcrumb]:text-neutral-500/80 sm:mb-0 z-2">
                ...
                <li><div 
                    ...
                    <a x-ref="content" href="{{ route('wall.show', $topic->wall) }}" class="inline-flex items-center py-1 font-normal hover:text-neutral-900 focus:outline-none z-10">{{ $topic->wall->name }}</a>
                </div></li>
                <span class="mx-2 text-gray-400">/</span>
                <li><div 
                    x-data="{
                        tooltipVisible: false,
                        tooltipText: 'Topic (This page)',
                        tooltipArrow: true,
                        tooltipPosition: 'top',
                    }"
                    x-init="$refs.content.addEventListener('mouseenter', () => { tooltipVisible = true; }); $refs.content.addEventListener('mouseleave', () => { tooltipVisible = false; });"
                    ...
                    <a x-ref="content" class="inline-flex items-center py-1 font-normal hover:text-neutral-900 focus:outline-none z-10">{{ $topic->name }}</a>
                </div></li>
            </ol>
        </nav>
