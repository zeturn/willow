<footer class="text-gray-600 body-font">
    <div class="container px-5 py-24 mx-auto">
        <div class="flex flex-wrap text-center md:text-left">

            <!-- 大标题 -->
            <div class="lg:w-1/4 md:w-1/2 w-full px-4">
                <h2 class="title-font font-bold text-gray-900 tracking-widest text-5xl mb-3">memeGit</h2>
                <p class="text-gray-500 text-base lg:text-3xl mt-2 md:text-left text-center">记录迷因的<br class="hidden lg:block">分布式版本控制系统</p>
            </div>

            <!-- 导航链接 -->
            <div class="lg:w-3/4 md:w-1/2 w-full px-4 flex flex-col md:flex-row justify-evenly">
                <div class="flex flex-col mb-10">
                    <h2 class="title-font font-medium text-gray-900 tracking-widest text-sm mb-3">EXPLORE</h2>
                    <nav class="list-none">
                        <li><a href="/workstation" class="text-gray-600 hover:text-gray-800">Workstation</a></li>
                        <li><a href="/entry" class="text-gray-600 hover:text-gray-800">Entries</a></li>
                        <li><a href="/wall" class="text-gray-600 hover:text-gray-800">Wall</a></li>
                        <li><a href="/topic" class="text-gray-600 hover:text-gray-800">Topics</a></li>
                    </nav>
                </div>

                <div class="flex flex-col mb-10">
                    <h2 class="title-font font-medium text-gray-900 tracking-widest text-sm mb-3">RESOURCES</h2>
                    <nav class="list-none">
                        <li><a href="/map" class="text-gray-600 hover:text-gray-800">Map</a></li>
                        <li><a href="/category" class="text-gray-600 hover:text-gray-800">Categories</a></li>
                        <li><a href="/trees" class="text-gray-600 hover:text-gray-800">Trees</a></li>
                        <li><a href="/dag" class="text-gray-600 hover:text-gray-800">DAG</a></li>
                    </nav>
                </div>

                <div class="flex flex-col mb-10">
                    <h2 class="title-font font-medium text-gray-900 tracking-widest text-sm mb-3">MEDIA</h2>
                    <nav class="list-none">
                        <li><a href="/medias" class="text-gray-600 hover:text-gray-800">Medias</a></li>
                        <li><a href="/albums" class="text-gray-600 hover:text-gray-800">Albums</a></li>
                    </nav>
                </div>
            </div>
        </div>
    </div>
    <div class="text-sm text-gray-500 bg-gray-100">
        <div class="container px-5 py-6 mx-auto flex items-center sm:flex-row flex-col">
            <a class="flex title-font font-medium items-center md:justify-start justify-center text-gray-900">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" class="w-10 h-10 text-white p-2 bg-indigo-500 rounded-full" viewBox="0 0 24 24">
                    <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"></path>
                </svg>
                <span class="ml-3 text-xl">HolllowData</span>
            </a>
            <p class="sm:ml-6 sm:mt-0 mt-4">© 2021-2023 memeGit — <a rel="noopener noreferrer" class="text-gray-600 ml-1" target="_blank">@Hollowdata Web</a></p>
            <span class="inline-flex sm:ml-auto sm:mt-0 mt-4 justify-center sm:justify-start">
                <!-- 社交媒体链接图标 -->
            </span>
        </div>
    </div>
</footer>
