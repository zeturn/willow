<header x-data="{ slideOverOpen: false }" class="text-gray-600 body-font">
    <div class="container mx-auto flex flex-wrap p-5 flex-col md:flex-row items-center">
        <!-- Branding and Mobile Menu Button -->
        <a class="flex title-font font-medium items-center text-blue-500 mb-4 md:mb-0" href="{{ url('/') }}">
            <span class="ml-3 text-3xl">memeGit</span>
        </a>
        <button @click="slideOverOpen=true" class="md:hidden ml-auto inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-gray-500 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 focus:text-gray-500 transition duration-150 ease-in-out">
        ...
        navigationMenuClose(){
            ...
            <!-- Your navigation links for desktop -->
            <!-- Placeholder for navigation links, you should replace it with your actual navigation content -->
            <div class="relative">
        ...
        @guest
            <button class="hidden md:inline-flex items-center border-0 py-1 px-4 focus:outline-none hover:bg-gray-200 rounded text-base mt-4 md:mt-0">
    ...
    <div x-show="slideOverOpen" @click.away="slideOverOpen = false" @keydown.window.escape="slideOverOpen = false" class="fixed inset-0 overflow-hidden z-50" style="display: none;">
        <div class="absolute inset-0 overflow-hidden">
            <!-- Background overlay, show/hide based on slide-over state. -->
            <div x-show="slideOverOpen" x-transition:enter="ease-in-out duration-500" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in-out duration-500" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="absolute inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true"></div>
            
            <section class="absolute inset-y-0 right-0 pl-10 max-w-full flex" x-transition:enter="transform transition ease-in-out duration-500" x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transform transition ease-in-out duration-500" x-transition:leave-start="translate-x-0" x-transition:leave-end="translate-x-full">
                <!-- Slide-over panel, show/hide based on slide-over state. -->
                <div class="relative w-screen max-w-md">
                    <!-- Panel content -->
                    <div class="flex h-full flex-col py-6 bg-white shadow-xl overflow-y-scroll">
                        <div class="px-4 sm:px-6">
                            <div class="flex items-start justify-between">
                                <h2 class="text-lg font-medium text-gray-900" id="slide-over-title">Menu</h2>
                                <div class="ml-3 h-7 flex items-center">
                                    <button @click="slideOverOpen = false" class="bg-white rounded-md text-gray-400 hover:text-gray-500 focus:ring-2 focus:ring-indigo-500">
                        ...
                        <div class="mt-6 relative flex-1 px-4 sm:px-6">
                            ...
                            <nav class="space-y-1">
                                <!-- Your navigation links for mobile, including Login/Register for guests or user menu for authenticated users -->
                            </nav>
                            <!-- /End replace -->
                        </div>
                    </div>
