        ...
        <div class="w-full lg:w-3/4 px-4">
            ...
            <nav class="flex justify-between">
                <ol class="inline-flex items-center mb-3 space-x-3 text-sm text-neutral-500 [&_.active-breadcrumb]:text-neutral-500/80 sm:mb-0">
                    ...
                    <li><a href="{{ route('entry.show.explanation', $version->branch->entry->id) }}" class="inline-flex items-center py-1 font-normal hover:text-neutral-900 focus:outline-none">{{ substr($version?->branch?->entry?->name, 0, 30) }}</a></li>
                    <span class="mx-2 text-gray-400">/</span>
                    <li><a href="{{ route('entry.branch.show.showDemoVersion', $version->branch->id) }}" class="inline-flex items-center py-1 font-normal hover:text-neutral-900 focus:outline-none">{{ substr($version?->branch?->name, 0, 30) }}</a></li>
                    <span class="mx-2 text-gray-400">/</span>
                    <li><a class="inline-flex items-center py-1 font-normal rounded cursor-default active-breadcrumb focus:outline-none">{{ substr($version?->name, 0, 30) }}</a></li>
                </ol>
            </nav>
