...
<div class="container mx-auto max-w-9xl p-4">
    <div class="flex flex-col md:flex-row md:w-10/12 mx-auto bg-white my-6 rounded-lg">
        ...
        <div class="md:w-1/2 p-8">
            ...
            <!-- Delete media form -->
            @can('media-delete')
                <form action="{{ route('medias.delete', $media->id) }}" method="POST">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="bg-color-danger-500 hover:bg-color-danger-600 text-red" onclick="return confirm('Are you sure you want to delete this media?');">
