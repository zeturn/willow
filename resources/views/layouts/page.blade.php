...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <div x-data="{
        ...
        "
        class="fixed top-0 left-0 w-full h-auto py-2 duration-300 ease-out bg-white dark:bg-gray-800 shadow-sm sm:py-0 sm:h-10 z-50" x-cloak>
        <div class="flex items-center justify-between w-full h-full px-3 mx-auto max-w-7xl z-50">
            <a href="#" class="flex flex-col w-full h-full text-xs leading-6 text-black dark:text-gray-400 duration-150 ease-out sm:flex-row sm:items-center opacity-80 hover:opacity-100">
                <span class="flex items-center">
                    ...
                    <strong class="font-semibold">注意！Notice!</strong><span class="hidden w-px h-4 mx-3 rounded-full sm:block bg-neutral-200"></span>
                </span>
                <span class="block pt-1 pb-2 leading-none sm:inline sm:pt-0 sm:pb-0">本网站上的图文内容来源于机器生成，不代表任何人的观点和立场。The graphic and textual content on this website is generated by machines and does not represent anyone's views or positions.</span>
            </a>
            <button @click="bannerVisible=false; setTimeout(()=>{ bannerVisible = true }, 1000);" class="flex items-center flex-shrink-0 translate-x-1 ease-out duration-150 justify-center w-6 h-6 p-1.5 text-black rounded-full hover:bg-neutral-100" aria-label="close">
