...
@section('result-list')
    ...
    @else
        @if($query == "everything")
            <p class="text-center text-gray-500 my-4">Everything is here.</p>
        @endif
        <ul class="divide-y divide-gray-200">
            @foreach ($results as $entry)
                <li class="py-4">
                    <a href="{{ route('entry.show.explanation', ['id' => $entry->id]) }}" class="block text-lg font-semibold text-gray-200">{{ $entry->name }}</a>
