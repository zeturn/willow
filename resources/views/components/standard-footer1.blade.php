<footer class="text-gray-600 body-font">
    <div class="container max-w-7xl py-24 mx-auto">
        <div class="flex flex-wrap text-center md:text-left">

            <!-- 大标题 -->
            <div class="lg:w-1/4 md:w-1/2 w-full">
                <h2 class="title-font font-bold tracking-tight leading-8 text-gray-900 dark:text-gray-300 text-5xl mb-3">{{config('app.name', 'memeGit')}}</h2>
                <p class="text-gray-500 text-base lg:text-3xl mt-2 md:text-left text-center"><br class="hidden lg:block"></p>
            </div>

            <!-- 导航链接 -->
            <div class="lg:w-3/4 md:w-1/2 w-full flex flex-col md:flex-row justify-evenly">
                <div class="flex flex-col mb-10">
                    <h2 class="title-font font-medium text-gray-900 dark:text-gray-300 tracking-widest text-sm mb-3">{{__('basic.Explore')}}</h2>
                    <menu class="list-none">
                        <li><a href="/" class="text-gray-600 hover:text-gray-800">{{__('basic.Index')}}</a></li>
                        <li><a href="{{ route('explore.entry_waterfall') }}" class="text-gray-600 hover:text-gray-800">{{__('basic.Entry')}}</a></li>
                        <li><a href="{{ route('explore.wall_square') }}" class="text-gray-600 hover:text-gray-800">{{__('basic.Wall')}}</a></li>
                        <li><a href="{{ route('explore.node_square') }}" class="text-gray-600 hover:text-gray-800">{{__('basic.Node')}}</a></li>
                    </menu>
                </div>

                <div class="flex flex-col mb-10">
                    <h2 class="title-font font-medium text-gray-900 dark:text-gray-300 tracking-widest text-sm mb-3">{{__('basic.Resource')}} </h2>
                    <menu class="list-none">
                        <li><a href="/map" class="text-gray-600 hover:text-gray-800">{{__('basic.Map')}}</a></li>
                        <li><a href="/category" class="text-gray-600 hover:text-gray-800">{{__('basic.Category')}}</a></li>
                        <li><a href="/trees" class="text-gray-600 hover:text-gray-800">{{__('basic.Tree')}}</a></li>
                        <li><a href="/dcg" class="text-gray-600 hover:text-gray-800">{{__('basic.DCG')}}</a></li>
                    </menu>
                </div>

                <div class="flex flex-col mb-10">
                    <h2 class="title-font font-medium text-gray-900 dark:text-gray-300 tracking-widest text-sm mb-3">{{__('basic.Media')}}</h2>
                    <menu class="list-none">
                        <li><a href="/medias" class="text-gray-600 hover:text-gray-800">{{__('basic.Media')}}</a></li>
                        <li><a href="/albums" class="text-gray-600 hover:text-gray-800">{{__('basic.Album')}}</a></li>
                    </menu>
                </div>
            </div>
        </div>
    </div>

    <div class="text-sm text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-800">
        <div class="container px-5 py-6 mx-auto flex items-center sm:flex-row flex-col">
            <a href="https://pacenotes.hollowdata.com" class="flex title-font font-medium items-center md:justify-start justify-center text-gray-900 dark:text-white">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" class="w-10 h-10 text-white p-2 bg-indigo-500 rounded-full" viewBox="0 0 24 24">
                    <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"></path>
                </svg>
                <span class="ml-3 text-xl">HollowData</span>
            </a>
            <p class="sm:ml-6 sm:mt-0 mt-4 text-gray-600 dark:text-gray-300">© 2021-2024 {{config('app.name', 'memeGit')}} — {{__('basic.An experimental project to study automated machine mining of popular culture')}}</p>
            <span class="inline-flex sm:ml-auto sm:mt-0 mt-4 justify-center sm:justify-start">
            {{config('app.name', 'memeGit')}} LB V0.9 (2024/5)
                <!-- 社交媒体链接图标 -->
            </span>
        </div>
    </div>


</footer>
