    ...
    <div class="flex flex-wrap -mx-4">
        <div class="w-full lg:w-3/4 px-4">
            <div class="bg-white rounded-lg p-6 dark:border-gray-700 dark:bg-gray-800 mb-4">
                <h1 class="text-lg font-bold mb-4">编辑边</h1>
                <form action="{{ route('edges.update', $edge->id) }}" method="POST" class="space-y-4">
                    @csrf
                    @method('PATCH')
                    <div>
                    ...
                    <button type="submit" class="px-4 py-2 bg-green-500 hover:bg-green-700 rounded-md text-white font-medium">更新</button>
                </form>
            </div>
        </div>
